...
public class ChatService {
	private static final int SUMMARY_THRESHOLD = 10000; // 토큰 기준 예시
	...
	public Flux<ServerSentEvent<String>> streamChat(String sessionId, String message) {
		...
		List<String> redisConversation = Optional.ofNullable(redisTemplate.opsForList().range(conversationKey, 0, -1))
			.orElse(new ArrayList<>());

		// // @TODO 요약 필요 시 SYSTEM 메시지 생성 + Redis 초기화 후 저장
		// if (clovaStudioService.getTokenCount(redisConversation, ClovaStudioClient.HCX_DASH_002) > SUMMARY_THRESHOLD) {
		// 	String summary = clovaStudioService.summarizeConversation(redisConversation);
		// 	redisTemplate.delete(conversationKey);
		// 	redisTemplate.opsForList().rightPush(conversationKey, "SUMMARY: " + summary);
		//  redisConversation = Optional.ofNullable(redisTemplate.opsForList().range(conversationKey, 0, -1)).orElse(new ArrayList<>());
		// }
		...
		return clovaStudioService.laundryChatbot(redisConversation, message)
			.filter(chunk -> !chunk.isBlank())
			.flatMapSequential(chunk -> {
				paragraphBuffer.append(chunk);    // 문단 누적
				...
				while ((index = paragraphBuffer.indexOf("\n\n")) >= 0) {
					...
					paragraphBuffer.delete(0, index + 2); // 처리 후 버퍼에서 제거
				}
